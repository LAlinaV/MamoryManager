...
private slots:
    void addProcess() {
        ...
        if (sizeOk && size > 0 && idOk && id >= 0) {
            ...
            char* data = new char[size - sizeof(int)];
            std::memset(data, '0', (size - sizeof(int)) * sizeof(char));
    ...
    void readData() {
        bool idOk;
        int id = ioProcessIdInput->toPlainText().toInt(&idOk);
        char data[4096];
        
        if (idOk && id >= 0) {
            Page pageToRead = manager.getPage(manager.getPages()[id][0]);
            manager.read_memory(data, pageToRead.address, 4096);
        }
        // Implement reading data logic here
        QMessageBox::information(this, "Read Data", data);
    }
    ...
    void writeData() {
        ...
        int id = ioProcessIdInput->toPlainText().toInt(&idOk);
        char data[4096] = {0};
        memcpy(data, ioDataInput->toPlainText().toStdString().c_str(), 4096);
        
        if (idOk && id >= 0) {
            Page page = manager.getPage(manager.getPages()[id][0]);
            manager.write_memory(page.address, data, 4096);
        }
